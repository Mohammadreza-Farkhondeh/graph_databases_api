...
const edgeRoutes = async (app, opts, done) => {

    app.post('/', async (request, reply) => {
        try {
            const { dbSession, body} = request;
            const { className, properties, to_rid, from_rid } = body;

            let result = dbSession.create("EDGE", className)
                .from(from_rid)
                .to(to_rid)
                .set(properties)
                .one();
